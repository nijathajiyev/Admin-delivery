...
function AddOffersForm(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      offersPostApi(values).then((res) => {
        let arr = [...state.offerSlice.data, values];
        console.log(arr);
  ...
  return (
    <div>
      ...
      <form onSubmit={formik.handleSubmit}>
        <AddForm>
          <p>{t("form.form offers description")}</p>
          <AddFormRight>
        ...
        <FormSubmitDiv>
          <button type="cancel" onClick={() => props.handleClose()}>
            {t("cancel")}
          </button>
